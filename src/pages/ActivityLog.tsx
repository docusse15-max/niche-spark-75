...
export default function ActivityLog() {
  ...
  const actionColors: Record<string, string> = {
    "status_alterado": "bg-amber-100 text-amber-800",
    "nota_adicionada": "bg-blue-100 text-blue-800",
    "lead_criado": "bg-green-100 text-green-800",
    "lead_exportado": "bg-purple-100 text-purple-800",
  };
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-[900px] mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="outline" size="sm" onClick={refresh}><RefreshCw className="h-4 w-4 mr-1" />Atualizar</Button>
        </div>
        ...
        {log.length === 0 ? (
          ...
          <div className="space-y-1">
            {log.map(entry => {
              ...
              return (
                <div key={entry.id} className="flex items-start gap-3 p-3 bg-card border rounded-lg">
                  <div className="text-xs text-muted-foreground whitespace-nowrap mt-0.5 w-[90px] shrink-0">{timeStr}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Badge className={`text-[10px] ${actionColors[entry.action] || "bg-gray-100 text-gray-800"}`}>{entry.action.replace("_", " ")}</Badge>
                      <span className="font-semibold text-sm">{entry.leadEmpresa}</span>
                      {entry.author && <span className="text-xs text-muted-foreground">por <span className="font-medium text-foreground">{entry.author}</span></span>}
                    </div>
